...
The system configured with command line parameters, environment and configuration file. 

##### Command line and environment 

| Command line  | Environment | Default      | Description            |
| ------------- | ----------- | ------------ | ---------------------- |
| --config / -f | CONFIG      | `config.yml` | configuration file     |
...
_see example of complete [config.yml](https://github.com/umputun/remark/blob/master/config.yml] with comments_

The minimal config should have:

- `url` - url pointing to your remark42 server, i.e. `https://demo.reamark42.com`. Can be defined via environment `REMARK_URL`
- `secret` - secret key, can be any long and hard-to-guess string. 
- `auth.providers` - at least one section defining oauth2 provider(s)


|                   | REMARK_URL           |                       | no    | url to remark42 server                         |
...
services:
    remark:
        ...
        volumes:
            ...
            - ./config.yml:/srv/config.yml          # mapping of config file
```
